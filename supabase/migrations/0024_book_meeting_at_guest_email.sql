-- book_meeting_at previously had no guest_email param, so it could never
-- carry the client's email through to the Google Meet invite the way
-- book_meeting_auto does. Needed now that the caller app pins specific
-- callers to a specific consultant (see src/app/caller/actions.ts) instead
-- of always auto-assigning, and that path still needs to invite the client.
drop function if exists book_meeting_at(uuid, uuid, timestamptz, int, text, text, text);

create function book_meeting_at(
  p_lead_id uuid,
...
revoke all on function book_meeting_at(uuid, uuid, timestamptz, int, text, text, text, text) from public, anon;
grant execute on function book_meeting_at(uuid, uuid, timestamptz, int, text, text, text, text) to authenticated;
