-- Extends the default bookable working day from 9am-8pm to 9am-8:30pm, so a
-- 60-minute meeting starting at 7:30pm has somewhere to fit. This only
-- changes the RPCs' hardcoded fallback (used when a consultant has zero
-- explicit availability_windows rows for the day) — a consultant with real
-- explicit hours is governed solely by those rows, which is why this
-- migration doesn't touch the availability_windows table itself; Sarah's
-- existing rows (the only consultant with real hours set as of this
-- migration) were extended by 30 minutes directly, one-time, via the admin
-- Supabase console rather than a migration, since that's data, not schema.
...
begin
  ...
  v_default_end := (v_local_date + time '20:30') at time zone 'Asia/Kolkata';
